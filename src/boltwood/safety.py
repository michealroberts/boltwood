...
class BoltwoodIIISafetyMonitorDeviceInterface(BaseSafetyMonitorDeviceInterface):
    ...
    def _poll(self) -> None:
        ...
        while not self._polling_thread_closing_event.is_set():
            try:
                # Retrieve the latest safety status from the device:
                self._poll_latest_safety_status()
    ...
    def _start_polling(self) -> None:
        ...
        self._polling_thread = Thread(
            target=self._poll,
            name=f"SafetyMonitorDevice-{self._id}-PollingThread",
            daemon=True,
        )
    ...
    def _poll_latest_safety_status(self) -> None:
        """
        ...
        if self._is_closed():
            raise RuntimeError(
                f"[Safety Monitor ID {self.id}]: Serial port {self._port} is closed."
            )
